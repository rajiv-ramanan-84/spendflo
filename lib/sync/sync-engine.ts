    ...
    await prisma.$transaction(async (tx) => {
        ...
        try {
          ...
          const existing = await tx.budget.findFirst({
            where: {
              ...
              // Only match active budgets (not soft-deleted)
              deletedAt: null
            },
          ...
          console.error(`[Sync Engine] Error processing budget at row ${i + 1}:`, error.message);
          result.errors.push({
            row: i + 1,
            error: error.message || 'Unknown error',
            severity: 'error'
      ...
      const existingBudgets = await tx.budget.findMany({
        where: {
          customerId,
          source: metadata.sourceType,
          // Only find active budgets (not soft-deleted)
          deletedAt: null
        }
      });
